...
const NEW_CONTENT = `import * as React from "react";
...
const ConnectionsScreen = ({ onBack, isDarkMode = true, language = 'English', connections = [], suggestedConnections = [], currentUserId = null, onConnectionsUpdated, onOpenProfile }) => {
  ...
  const getFilteredPeople = () => {
    ...
    if (activeTab === 0) {
      ...
      filtered = people.filter(p => p.status === "Connected" && p.userRole === "mentor");
    } else if (activeTab === 3) {
...
fs.writeFileSync('c:/Users/ASUS/OneDrive/Desktop/skill genome/skillgenome-mobile/screens/ConnectionsScreen.js', NEW_CONTENT);
console.log('Replaced completely.');
