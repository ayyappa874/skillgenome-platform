...
const Screen11Web = (props) => {
  ...
  const drawRadar = () => {
    ...
    const polyLevels = [0.25, 0.5, 0.75, 1].map((f, idx) => {
      ...
      return (
        <polygon key={`poly-${idx}`} points={pts} fill="none" stroke="var(--border-strong)" strokeWidth={f===1 ? 2 : 1} strokeDasharray={f===1 ? "none" : "4 4"} />
      );
    });
    ...
    return (
      <svg viewBox="0 0 350 350" id="radarSvg" style={{ filter: 'drop-shadow(0 0 20px rgba(139, 107, 255, 0.2))' }}>
        <defs>
          <radialGradient id="radarFill" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
            <stop offset="0%" stopColor="#8b6bff" stopOpacity="0.8" />
        ...
        {/* Outer glowing ring */}
        <circle cx={cx} cy={cy} r={R + 15} fill="none" stroke="var(--violet)" strokeWidth={1} opacity={0.3} style={{ animation: 'pulseRing 3s infinite alternate' }} />
        <circle cx={cx} cy={cy} r={R + 10} fill="none" stroke="var(--teal)" strokeWidth={0.5} opacity={0.2} style={{ animation: 'pulseRing 4s infinite alternate-reverse' }} />
        ...
        {currentModules.map((m, i) => {
          ...
          return <circle key={`dot-${i}`} cx={x} cy={y} r={6} fill={m.accent} stroke="#fff" strokeWidth={2} style={{ filter: 'drop-shadow(0 0 4px rgba(255,255,255,0.8))' }} />;
        })}
      </svg>
    );
  };
