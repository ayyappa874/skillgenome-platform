...
function ProgressBar({ value, color, S, T }) {
  const anim = React.useRef(new Animated.Value(0)).current;
  React.useEffect(() => {
    Animated.timing(anim, { toValue: value, duration: 800, delay: 300, useNativeDriver: false }).start();
  }, [value]);
  return (
    <View style={[S.pbTrack, { backgroundColor: T.subtle }]}>
      <Animated.View style={[S.pbFill, {
...
const ProfileScreen = ({
  ...
  githubSkills = [], githubAnalysis = null, thoughtAnalysis = null,
  emotionAnalysis = null, recordingDuration = 0, journalEntries = [],
        ...
        <Animated.View style={{ opacity: fade, transform: [{ translateY: slide }], gap: 20 }}>
          ...
          <View style={S.hero}>
            ...
            <View style={S.heroBadgeRow}>
              ...
              {profile.target_role ? (
                <View style={S.heroBadge}>
                  <Text style={S.heroBadgeText}>🎯 Target: {profile.target_role}</Text>
                </View>
              ) : null}
              {experience > 0 ? (
                <View style={S.heroBadge}>
              ...
              {MODULES.map((m) => {
                ...
                return (
                  <View key={m.label} style={S.modBarRow}>
                    ...
                    <ProgressBar value={sc} color={m.accent} S={S} T={T} />
...
const getStyles = (T, isDarkMode) => StyleSheet.create({
  ...
  pbTrack: { height: 4, borderRadius: 2, overflow: "hidden", flex: 1 },
