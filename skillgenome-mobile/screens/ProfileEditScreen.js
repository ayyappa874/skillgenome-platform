...
const ProfileEditScreen = ({ onBack, profile = {}, onSaveProfile, isDarkMode = true, language = 'English' }) => {
  ...
  const handleSave = () => {
    if (typeof onSaveProfile === 'function') {
      onSaveProfile({ ...profile, name, title, target_role: targetRole, bio, location });
    ...
    <View style={[S.root, { backgroundColor: T.bg }]}>
      ...
      <ScrollView contentContainerStyle={S.content} showsVerticalScrollIndicator={false}>
        <Animated.View style={{ opacity: fade, transform: [{ translateY: slide }], gap: 16 }}>
          ...
          <View style={S.fieldGroup}>
            <Text style={[S.label, { color: T.muted }]}>TARGET ROLE</Text>
            <TextInput
              style={[S.input, { color: T.text, backgroundColor: T.surface, borderColor: T.border }]}
              value={targetRole}
