const fs = require('fs');
const path = 'c:/Users/ASUS/OneDrive/Desktop/skill genome/skillgenome-mobile/components/CohortRosterModal.js';
let content = fs.readFileSync(path, 'utf8');

// 1. Update fetchRoster to fetch timelines
const oldFetchRoster = `  const fetchRoster = async () => {
    try {
      setLoading(true);
          ...
          data.cohort_students.forEach(cs => {
            ...
            if (p) {
              mapped.push({
                ...
                avatar: p.avatar_url,
                score: p.genome_score || Math.floor(Math.random() * 25) + 70
              });
            }
          });
        }
...
const newFetchRoster = `  const fetchRoster = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase
        .from('cohorts')
...
content = content.replace(oldFetchRoster, newFetchRoster);

// 2. Update JSX to render the timeline
const oldCard = `              {students.map((student) => (
                <View key={student.id} style={S.studentCard}>
                  ...
                  <View style={S.studentInfo}>
                    ...
                    <Text style={S.studentScore}>Genome Match: {student.score}%</Text>
                  </View>
...
const newCard = `              {students.map((student) => (
...
content = content.replace(/studentCard: {[\s\S]*?messageBtnText: {[\s\S]*?}/, newStyles);

fs.writeFileSync(path, content, 'utf8');
console.log("Updated Roster Modal with Timelines");
