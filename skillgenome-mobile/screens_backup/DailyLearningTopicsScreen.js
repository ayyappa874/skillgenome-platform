...
const DailyLearningTopicsScreen = ({ onBack, isDarkMode = true, language = "English", jobMatch }) => {
  ...
  const loadRoadmap = React.useCallback(async (skill) => {
    ...
    setRoadmap([]);
    try {
      const progressKey = `${PROGRESS_KEY_PREFIX}${skill.replace(/\s+/g, "_")}`;
      ...
      // 1. Check knowledge base FIRST — instant, real, specific content
      ...
      // 2. Check cache for previously AI-generated roadmaps
...
Return ONLY valid JSON array of exactly 25 objects. Each: { "id": 1-25, "day": 1-5, "dayTitle": "4 word day theme", "level": "Fresher|Beginner|Intermediate|Advanced|Proficient", "subtopicTitle": "specific topic name max 5 words", "subtopicDesc": "one sentence about what this specific concept is and how it is applied in ${skill}" }`;
  ...
  // ── Persist progress ─────────────────────────────────────────────────────
  React.useEffect(() => {
    if (!selectedSkill || completedIds === null) return;
    const progressKey = `${PROGRESS_KEY_PREFIX}${selectedSkill.replace(/\s+/g, "_")}`;
    AsyncStorage.setItem(progressKey, JSON.stringify({ completed: completedIds, dateKey: todayKey })).catch(() => {});
  }, [completedIds, selectedSkill, todayKey]);
  ...
  const openLesson = async (subtopic) => {
    ...
    setLessonContent("");
    try {
      const cacheKey = `LESSON_${selectedSkill}_${subtopic.id}`.replace(/\s+/g, "_");
      const cached = await AsyncStorage.getItem(cacheKey);
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {isLoadingRoadmap ? (
          <View style={styles.loadingBox}>
            ...
            <Text style={styles.loadingText}>Building your personalized {selectedSkill} roadmap...</Text>
            <Text style={styles.loadingSubText}>AI is generating 25 specific subtopics for your skill</Text>
          </View>
        ) : (
          <>
            {/* Day selector tabs */}
              ...
              <View style={styles.dayTabs}>
                {days.map((d) => {
                  ...
                  return (
                    <Pressable key={d.day}
                      onPress={() => {
                        if (!unlocked) { Alert.alert("Day Locked 🔒", "Complete all topics in the previous day first to unlock this day."); return; }
                        setActiveDay(d.day);
                      }}
                ...
                <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
                  {lessonLoading ? (
                    <View style={styles.lessonLoading}>
                      <ActivityIndicator size="large" color="#25E0B5" />
                      <Text style={styles.lessonLoadingText}>AI is generating your lesson on "{selectedSubtopic.subtopicTitle}"...</Text>
...
const getStyles = (isDarkMode) => {
  ...
  return StyleSheet.create({
    ...
    modalSheet: { backgroundColor: card, borderTopLeftRadius: 32, borderTopRightRadius: 32, maxHeight: "90%" },
