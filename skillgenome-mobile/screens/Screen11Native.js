...
const QUICK_ACTIONS = [
  ...
  { id: "jobs", label: "Explore jobs", icon: "💼", colorKey: "amber", key: "onOpenJobMatches" },
  ...
  React.useEffect(() => {
    ...
    const fetchResources = async () => {
      try {
        ...
        let mentorIds = cohortsData?.map(c => c.cohorts?.mentor_id).filter(Boolean) || [];
        
        // 2. Get from direct mentorship requests
        const { data: requestsData } = await supabase.from('mentorship_requests').select('mentor_id').eq('student_id', profile.id).eq('status', 'accepted');
        if (requestsData) mentorIds = [...mentorIds, ...requestsData.map(r => r.mentor_id)];
        ...
        // Placeholder/Fallback logic: if no direct mentor resources are found
        if (fetchedData.length === 0) {
          const { data: fallbackData } = await supabase.from('mentor_resources').select('*').order('created_at', { ascending: false }).limit(2);
          if (fallbackData && fallbackData.length > 0) {
            fetchedData = fallbackData;
          } else {
            fetchedData = [{ id: 'mock-1', title: 'Mentor Resource (Placeholder)', type: 'PDF', url: 'https://react.dev' }];
          }
        }

        setMentorResources(fetchedData);
      } catch (e) { 
        console.warn('Error fetching mentor resources:', e); 
        // Emergency fallback if network fails, displaying the error message
        setMentorResources([{ id: 'mock-error', title: String(e.message || e), type: 'PDF', url: 'https://react.dev' }]);
      }
    };
    fetchResources();
  }, [profile?.id]);

  const openResource = async (url, title = 'download', ext = 'pdf') => {
    try {
      if (!url) return;
      if (url.startsWith('data:')) {
        ...
        const fileUri = FileSystem.cacheDirectory + `${safeTitle}.${ext}`;
      ...
      console.warn('Cannot open url:', url, e);
    }
  };
    ...
    <View style={[styles.root, { backgroundColor: C.bg }]}> 
        ...
        <Animated.View style={[styles.container, { maxWidth: isWide ? 1120 : "100%", transform: [{ translateY: slide }] }]}> 
          ...
          <View style={[styles.actionsGrid, isWide && { justifyContent: "flex-start", gap: 16 }]}>
            {[
                ...
                return (
                  <PressCard key={a.id} onPress={() => a.isResourceFolder ? setShowResources(true) : nav(actionHandlers[a.key])} style={[styles.actionCard, { width: isWide ? "15%" : "31%", borderColor: C.border, backgroundColor: C.surface }]}> 
            ...
            {skills.length > 0 && (
              <>
                ...
                <View style={styles.skillsWrap}>
                  {skills.map((s, i) => {
                    const accents = [C.violet, C.cyan, C.rose, C.green, C.amber, C.purple];
                    const modColor = accents[i % accents.length];
                    return (
                      <View key={s.name} style={[styles.skillPill, { backgroundColor: `${modColor}12`, borderColor: `${modColor}30` }]}> 
                        <Text style={[styles.skillPillText, { color: modColor }]}>{s.name}</Text>
                        <Text style={[styles.skillPillScore, { color: C.text }]}>{s.score}</Text>
                      </View>
                    );
                  })}
                </View>
      ...
      {showResources && (
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end', zIndex: 9999 }}>
          <View style={{ backgroundColor: C.surface, borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 24, maxHeight: '80%', paddingBottom: 40 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <Text style={{ fontSize: 22, fontWeight: '800', color: C.text }}>Mentor Resources</Text>
              <Pressable onPress={() => setShowResources(false)} style={{ width: 36, height: 36, backgroundColor: C.surface2, borderRadius: 18, alignItems: 'center', justifyContent: 'center' }}>
              ...
              {mentorResources.map(res => (
                <Pressable 
                  ...
                  onPress={() => { 
                    ...
                    openResource(res.url, res.title, res.type === 'PDF' ? 'pdf' : res.type === 'Video' ? 'mp4' : 'doc'); 
                  }}
                  style={{ flexDirection: 'row', alignItems: 'center', padding: 16, backgroundColor: C.surface2, borderRadius: 16, marginBottom: 12, borderWidth: 1, borderColor: C.border }}
                >
                  <View style={{ width: 48, height: 48, borderRadius: 12, backgroundColor: `${C.amber}20`, alignItems: 'center', justifyContent: 'center', marginRight: 16 }}>
                    <Text style={{ fontSize: 24 }}>{res.type === 'PDF' ? '📄' : res.type === 'Video' ? '🎬' : '🔗'}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: C.text, fontSize: 16, fontWeight: '700' }}>{res.title}</Text>
                    <Text style={{ color: C.muted, fontSize: 13, marginTop: 4 }}>{res.type} • Shared by Mentor</Text>
                  </View>
                  <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 6, backgroundColor: C.amber, borderRadius: 20 }}>
                    <Feather name="download" size={14} color="#fff" style={{ marginRight: 4 }} />
                    <Text style={{ color: '#fff', fontSize: 12, fontWeight: '800' }}>Download</Text>
                  </View>
                </Pressable>
              ))}
            </Animated.ScrollView>
