...
const hookEnd = `    settings: () => { setActiveNav("settings"); nav(onOpenSettings); },
  };`;
...
if (hookStartIndex !== -1 && hookEndIndex !== -1) {
  ...
  content = content.substring(0, hookStartIndex) + replacement + content.substring(hookEndIndex + hookEnd.length);
