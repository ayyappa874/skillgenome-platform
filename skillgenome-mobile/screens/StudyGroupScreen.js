...
const StudyGroupScreen = ({ 
  ...
  const group = studyGroup ? { 
    ...studyGroup,
    members: studyGroup.members && Array.isArray(studyGroup.members) ? studyGroup.members : ["Y"]
  } : { name: "Study Group", memberCount: 1, onlineCount: 1, members: ["Y"] };
  const messages = studyGroupMessages && Array.isArray(studyGroupMessages) ? studyGroupMessages : [];
  const connections = connectionsProp && Array.isArray(connectionsProp) ? connectionsProp : [];
  ...
  return (
    <View style={[styles.container, { backgroundColor: isDarkMode ? "#060612" : "#f1f5f9" }]}>
      ...
      {/* Header */}
      <View style={[styles.header, { backgroundColor: isDarkMode ? 'transparent' : headerBg }]}>
      ...
      <View style={styles.membersRow}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          {group.members.map((memberItem, i) => {
            const nameStr = typeof memberItem === 'string' ? memberItem : (memberItem && memberItem.name ? memberItem.name : "U");
            const initials = nameStr === "You" ? "Y" : (nameStr.charAt ? nameStr.charAt(0).toUpperCase() : "U");
            return (
              <View key={i} style={[styles.memberAvatar, { backgroundColor: nameStr === "You" ? "#9B5BFF" : "#3b82f6" }]}> 
              ...
              <View style={[styles.messageRow, message.isOwn && styles.messageRowOwn]}>
                ...
                {message.resource ? (
                  ...
                  <View style={[styles.messageBubble, { backgroundColor: cardBg }, message.isOwn && styles.messageBubbleOwn]}>
                    ...
                    <Text style={[styles.messageTime, { color: textSecondary }, message.isOwn && styles.messageTimeOwn]}>{getMessageTime(message.time)}</Text>
                  </View>
                )}
              </View>
            </View>
