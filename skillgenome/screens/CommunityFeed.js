...
const CommunityFeed = ({
  ...
  onOpenStudyGroup,
  posts = [], connections = [], studyGroups = [], userRank = null,
  ...
  const renderLeftPanel = () => (
    <View style={{ gap: 24 }}>
      <View style={S.panelSection}>
        ...
        {connections.slice(0, 5).map(m => (
          <View key={m.id} style={S.suggestedCard}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
              <View style={[S.avatarSm, { backgroundColor: m.tone ? m.tone[0] : T.purple }]}><Text style={S.avatarSmText}>{m.name ? m.name[0] : 'U'}</Text></View>
              <View style={{ flex: 1 }}>
                <Text style={S.memberName}>{m.name}</Text>
                <Text style={S.memberRole}>{m.role || 'Member'}</Text>
              </View>
            </View>
            <View style={S.matchReasonBox}>
              <Text style={S.matchReasonText}>{m.sharedSkills?.length || 0} shared skills · Genome {m.match || 0}%</Text>
            </View>
            <Pressable style={S.connectBtn}><Text style={S.connectBtnText}>Connect</Text></Pressable>
          </View>
        ))}
        {connections.length === 0 && (
           <Text style={{color: T.muted, fontSize: 13}}>No suggestions right now. Keep exploring!</Text>
        )}
      </View>
      <View style={S.panelSection}>
