...
import {
  ...
  Pressable, Animated, useWindowDimensions, Platform
} from "react-native";
...
function GlowBar({ value, color, C }) {
...
function ScoreRing({ score, accent, C, isWide }) {
  const numAnim = React.useRef(new Animated.Value(0)).current;
  const [display, setDisplay] = React.useState(0);
  React.useEffect(() => {
    numAnim.addListener(({ value }) => setDisplay(Math.round(value)));
    Animated.timing(numAnim, { toValue: score, duration: 1000, delay: 200, useNativeDriver: false }).start();
    return () => numAnim.removeAllListeners();
  }, [score]);

  const size = isWide ? 132 : 116;
  return (
    <View style={[styles.ringWrap, { width: size, height: size }]}> 
      <LinearGradient colors={[accent || C.violet, C.teal]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.ringOuter, { borderRadius: size / 2 }]}> 
        <View style={[styles.ringInner, { backgroundColor: C.bg, borderRadius: (size - 8) / 2, width: size - 8, height: size - 8 }]}> 
          <Text style={[styles.ringNumber, { color: C.text }]}>{display}</Text>
          <Text style={[styles.ringUnit, { color: C.muted }]}>/100</Text>
        </View>
      </LinearGradient>
    </View>
  );
    ...
    <View style={[styles.root, { backgroundColor: C.bg }]}> 
      ...
      <Animated.ScrollView style={{ flex: 1, opacity: fade }} contentContainerStyle={[styles.scrollContent, { paddingBottom: 150 }]} showsVerticalScrollIndicator={false}>
        <Animated.View style={[styles.container, { maxWidth: isWide ? 1120 : "100%", transform: [{ translateY: slide }] }]}> 
          <View style={styles.header}>
            ...
            <Pressable style={styles.profileBtn} onPress={() => nav(onOpenProfile)}>
              <View style={styles.headerName}>
                <Text style={[styles.nameText, { color: C.text }]}>{profile.name || "Ayyappa"}</Text>
          ...
          <View style={[styles.heroCard, { backgroundColor: C.surface, borderColor: C.borderStrong }]}> 
            ...
            <View style={[styles.heroBody, { flexDirection: isWide ? "row" : "column" }]}> 
              <ScoreRing score={genomeScore} accent={C.violet} C={C} isWide={isWide} />
                  ...
                  {MODULES.map((m) => {
                    ...
                    return (
                      <View key={m.id} style={styles.miniBarItem}>
                        <Text style={[styles.miniBarLabel, { color: done ? modColor : C.muted }]}>{m.icon} {m.label}</Text>
                        <GlowBar value={done ? score : 0} color={modColor} C={C} />
          ...
          <View style={styles.companyGrid}>
            {TOP_COMPANIES.map((company, index) => (
              <PressCard key={`${company.name}-${index}`} onPress={() => nav(onOpenExplore)} style={[styles.companyCard, { backgroundColor: C.surface, borderColor: C.border }]}> 
                <View style={styles.companyTopRow}>
                  <View style={[styles.companyLogo, { backgroundColor: `${company.accent}16` }]}> 
                    <Text style={[styles.companyLogoText, { color: company.accent }]}>{company.name.slice(0, 2).toUpperCase()}</Text>
                  </View>
                ...
                <View style={styles.companyMetaRow}>
                  <Text style={[styles.companyMeta, { color: C.muted }]}>{company.region}</Text>
                  <Text style={[styles.companyMeta, { color: C.muted }]}>Priority {index + 1}</Text>
                </View>
              </PressCard>
            ))}
          </View>
          ...
          <View style={styles.actionsGrid}>
            {QUICK_ACTIONS.map((a) => {
              const modColor = C[a.colorKey];
              return (
                <PressCard key={a.id} onPress={() => nav(actionHandlers[a.key])} style={[styles.actionCard, { width: isWide ? "15%" : "31%", borderColor: C.border, backgroundColor: C.surface }]}> 
          ...
          {skills.length > 0 && (
            <>
              ...
              <View style={styles.skillsWrap}>
                {skills.map((sk, idx) => {
                  const accents = [C.violet, C.teal, C.cyan, C.rose, C.amber, C.green];
                  const a = accents[idx % accents.length];
                  return (
                    <View key={sk.name || idx} style={[styles.skillPill, { borderColor: `${a}44`, backgroundColor: `${a}12` }]}> 
                      <Text style={[styles.skillPillText, { color: a }]}>{sk.name}</Text>
                      {sk.score != null && (
                        <View style={[styles.skillPillBadge, { backgroundColor: `${a}22` }]}> 
                          <Text style={[styles.skillPillScore, { color: a }]}>{sk.score}%</Text>
                        </View>
                      )}
                    </View>
                  );
      ...
      <View style={styles.navContainer}>
        {Platform.OS === "web" ? (
          <View style={[styles.navBlurFallback, { backgroundColor: `${C.surface}cc`, borderColor: C.borderStrong }]}> 
            <NavContent activeNav={activeNav} navHandlers={navHandlers} C={C} />
          </View>
        ) : (
...
const NavContent = ({ activeNav, navHandlers, C }) => (
  <View style={styles.navContentRow}>
    {[
      { id: "home", icon: "⌂", label: "Home", onPress: navHandlers.home },
      { id: "explore", icon: "⊕", label: "Explore", onPress: navHandlers.explore },
    ].map((item) => (
      <Pressable key={item.id} style={styles.navItem} onPress={item.onPress}>
        <Text style={[styles.navIcon, { color: activeNav === item.id ? C.violet : C.muted }]}>{item.icon}</Text>
        <Text style={[styles.navLabel, { color: activeNav === item.id ? C.violet : C.muted }]}>{item.label}</Text>
      </Pressable>
    ))}
    ...
    {[
      { id: "community", icon: "⊘", label: "Network", onPress: navHandlers.community },
      { id: "settings", icon: "◎", label: "Settings", onPress: navHandlers.settings },
    ].map((item) => (
      <Pressable key={item.id} style={styles.navItem} onPress={item.onPress}>
        <Text style={[styles.navIcon, { color: activeNav === item.id ? C.violet : C.muted }]}>{item.icon}</Text>
        <Text style={[styles.navLabel, { color: activeNav === item.id ? C.violet : C.muted }]}>{item.label}</Text>
...
const styles = StyleSheet.create({
  ...
  navContentRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-around", paddingVertical: 10, paddingHorizontal: 8 },
  navItem: { flex: 1, alignItems: "center", gap: 4, paddingVertical: 3 },
  navIcon: { fontSize: 18 },
  navLabel: { fontSize: 10, fontWeight: "700", letterSpacing: 0.2 },
  navCenter: { alignItems: "center", justifyContent: "center", paddingHorizontal: 10 },
  navCenterBtn: { width: 54, height: 54, borderRadius: 27, alignItems: "center", justifyContent: "center", marginTop: -18, shadowColor: "#7c3aed", shadowOpacity: 0.4, shadowRadius: 14, shadowOffset: { width: 0, height: 8 } },
