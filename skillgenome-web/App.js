...
const App = () => {
        ...
        if (updates.resumeAnalysis) {
          try {
            const { error } = await supabase.from('resume_analyses').insert([
              { user_id: user.id, analysis_data: updates.resumeAnalysis }
            ]);
            if (error) console.warn("Supabase resume insert error:", error);
  ...
  const syncUserData = async (userId) => {
    ...
    try {
      ...
      await Promise.all([
        ...
        fetchThoughtTask,
        fetchGithubTask
      ]).then(async ([_j, emotionData, _u, _c, _s, _l, _sc, _mod, _score, resumeData, thoughtData, githubData]) => {

        const resumeAnalysis = (resumeData && resumeData.data && resumeData.data.length > 0) ? resumeData.data[0].analysis_data : null;
        const thoughtAnalysis = (thoughtData && thoughtData.data && thoughtData.data.length > 0) ? thoughtData.data[0].analysis_data : null;
        const githubAnalysis = (githubData && githubData.data && githubData.data.length > 0) ? githubData.data[0].analysis_data : null;
        const emotionAnalysisLoaded = (emotionData && emotionData.data && emotionData.data.length > 0) ? emotionData.data[0].analysis_data : null;
  ...
  React.useEffect(() => {
    const checkSessionAndInitialize = async () => {
      try {
        ...
        // Wrap Supabase session retrieval with a 1.5s timeout to prevent hanging on network/DNS errors
        const sessionPromise = supabase.auth.getSession();
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error("Supabase session retrieval timed out")), 1500)
        );
  ...
  const handleSignInSubmit = async (email, password) => {
    ...
    if (email.toLowerCase() === 'ayyappa@test.com') {
      ...
      setCurrentScreen(9); // Route through Permissions Screen first
      return;
    }
  ...
  const fetchSuggestedConnections = async () => {
    try {
      ...
      const { data: reqData } = await supabase.from('mentorship_requests').select('*').or(`student_id.eq.${user.id},mentor_id.eq.${user.id}`);
      ...
      const myProfile = await supabase.from('profiles').select('skills, title, role').eq('id', user.id).single();
      ...
      if (allProfiles && allProfiles.length > 0) {
        const mapped = allProfiles.map((profile) => {
          ...
          const myScoreRow = genomeScores.find(s => s.user_id === user.id);
          ...
          if (req) {
            if (req.status === 'accepted') status = "Connected";
               ...
               status = (req.mentor_id === user.id) ? "Accept" : "Pending";
            }
          }
          ...
          if (reqData) {
            reqData.forEach(r => {
              if (r.status === 'Connected') {
                myConnections.add(r.student_id === user.id ? r.mentor_id : r.student_id);
              }
            });
          }
          myConnections.add(user.id); // Self
  ...
  const handleAddComment = async (postId, text, parentId = null) => {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        const payload = { post_id: postId, content: text, author_id: user.id };
    ...
    try {
      ...
      if (data) {
        return data.map(c => ({
          ...
          handle: '@' + (c.profiles?.name || 'anonymous').toLowerCase().replace(/\s+/g, ''),
  ...
  const handleDeleteGroup = async () => {
    try {
      ...
      const { error } = await supabase.from('study_groups').delete().eq('id', activeStudyGroupId);
      if (error) throw error;
      await fetchStudyGroups();
      setStudyGroup(null);
      ...
      <View style={[isMobileWebViewport ? styles.webScreen : styles.desktopScreen, { backgroundColor: darkMode ? "#09090b" : "#f8fafc" }]}>
        ...
        {currentScreen === 10 && profile?.role === 'mentor' ? (
          ...
          <Screen11
            ...
            onOpenInterviewPrep={() => setCurrentScreen(37)}
            onOpenProfile={() => setCurrentScreen(41)}
        ...
        {currentScreen === 11 && (
          <TimelineScreen
            ...
            onOpenAlerts={() => setCurrentScreen(12)}
            profile={profile}
            resumeAnalysis={resumeAnalysis}
            githubAnalysis={githubAnalysis}
            thoughtAnalysis={thoughtAnalysis}
            ...
            onOpenCareerStimulation={() => setCurrentScreen(31)}
            onOpenJobMatches={() => setCurrentScreen(34)}
            isDarkMode={darkMode} language={language}
          />
        )}
        ...
        {currentScreen === 61 && (
          <EmotionPrintInputScreen
            onBack={() => setCurrentScreen(Platform.OS === 'web' ? 11 : 10)}
            onComplete={async (result) => {
        ...
        {currentScreen === 23 && (
          <CommunityFeed
            ...
            onOpenStudyGroup={(group) => {
              setActiveStudyGroupId(group.id);
              setStudyGroup(group);
              fetchStudyGroupMessages(group.id);
              setCurrentScreen(36);
            }}
            onOpenJobMatches={() => setCurrentScreen(34)}
            onOpenPost={(post) => {
            ...
            userRank={{ total_score: profile.genomeScore || 50 }}
            setPosts={setPosts}
        ...
        {currentScreen === 36 && (
          <StudyGroupScreen
            onBack={() => setCurrentScreen(23)}
            connections={connectionsForStudy}
            onSendInvites={(selectedIds) => {
              const invited = connectionsForStudy.filter(c => selectedIds.includes(c.id)).map(c => c.name);
